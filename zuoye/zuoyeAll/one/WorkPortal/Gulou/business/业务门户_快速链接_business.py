...
class Home:
    ...
    def assert_title(self, title_name=None):
        ...
        if title_name != None:
            ...
            return get_title(self.browser.driver)

    def switch_windows(self, title_name=None):  # title的名称
        handl_list = self.browser.driver.window_handles
        current_handle = self.browser.driver.current_window_handle
        for i in handl_list:
            if i != current_handle:
                time.sleep(2)
                self.browser.driver.switch_to.window(i)
                if self.assert_title(title_name):  # 当前窗口title_name
                    break


    def quick_links_add(self):   # 快速链接 新增
    ...
    def quick_links_click(self):  #快速链接点击，跳转
        ...
        sleep(2)
        # 切换至新的 窗口
        self.switch_windows(title2)  #切换到 title2
        sleep(3)
    ...
    def quick_links_edit(self):  #编辑快速链接
        self.browser.driver.implicitly_wait(5)
        sleep(2)
        name_edit = "编辑百科"  + date
        print("首页，快速链接,编辑 ： $ ")
    ...
    def quick_links_delete(self):  #删除快速链接
        self.browser.driver.implicitly_wait(5)
        sleep(2)
        print("首页，快速链接,验证链接 ： $ ")
        try:
