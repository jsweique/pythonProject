...
class Login:
    ...
    def image(self, src):
        ...
        # print(size)
        rangle = (int(location['x'] +1320), int(location['y'] +570),
                  int(location['x'] + size['width'] +1430),
                  int(location['y'] + size['height'] + 610))  # 写成我们需要截取的位置坐标(前两个代表定位位置，第三个代表离第一个点的宽度，第四个代表离第一个点的高度)
    ...
    def login(self,username,password):
        ...
        while Flag:
            ...
            if i ==11:
                break
            image = self.image(r'D:\yanzhengma')
            time.sleep(1)
